...
def generate_report_pdf(best_name, test_rmse, top3_list, top_features, feature_importance_img=None, final_data=None):
    ...
    # --- PAGE 2: SUMMARY & BACKGROUND & DATA FRAMEWORK (Page Limit Guarded) ---
    pdf.add_page()
    ...
    # --- PAGE 3: RESULTS, TABLE, AND ROI DIAGNOSTICS (Page Limit Guarded) ---
    pdf.add_page()
    ...
    # --- PAGE 4: PLAN & FOOTER & RENEWABLES ROADMAP (Page Limit Guarded) ---
    pdf.add_page()
    ...
    html_page4_sub = f"""
    <b>□ 핵심 영향 변수에 대한 기술적 제언 (스마트 로드 밸런싱)</b>
    <ul>
